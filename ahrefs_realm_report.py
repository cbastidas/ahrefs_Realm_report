...
class TokenAnalyzerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Token Analyzer Realm")
    ...
    def fetch_api_data(self, tokens):
        url = f"https://admin2.neataffiliates.com/feeds.php?FEED_ID=4&TOKENS={','.join(tokens)}"
        try:
            print(f"Sending tokens to API: {tokens}")
            response = requests.get(url, auth=HTTPBasicAuth('Token API', 'ToKenChAnGeMePaS$1234'))
        ...
        try:
            ...
            for token_element in root.findall(".//TOKEN"):
                ...
                data[token] = {
                    ...
                    'access_url': f"https://admin2.neataffiliates.com/affiliate_summary.php?id={setup_element.get('OBJECT_ID')}" if setup_element is not None else '',
